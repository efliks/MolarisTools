...
class EVBDatFile (object):
    ...
    def _ParseVMDFile (self, filename, absolute=False):
    ...
    def ReadVMDFiles (self, location=_VMD_DIR, logging=True, absoluteTorsions=False):
        ...
        if hasattr (self, "torsions"):
            ...
            for torsion in self.torsions:
                ...
                if os.path.exists (filename):
                    if logging:
                        print ("# . %s> Parsing file %s..." % (_MODULE_LABEL, filename))
                    values  = self._ParseVMDFile (filename, absolute=absoluteTorsions)
